...
# set the dataset dir
[[ -z $DATADIR ]] && DATADIR='/data/adlintul/input/ak8'
...
batchopts="--batch-size 256 --start-lr 5e-3"
...
$CMD \
    ...
    --model-prefix ${OUTPUT}/ak8_flavour_{auto}${suffix}/net \
    --gpus "2" \
    $dataopts $batchopts \
    --samples-per-epoch ${samples_per_epoch} --samples-per-epoch-val ${samples_per_epoch_val} \
    --num-epochs ${epochs} \
    --optimizer ranger --log ${OUTPUT}/ak8_flavour_{auto}${suffix}.log --predict-output pred.root \
    "${@:2}"
